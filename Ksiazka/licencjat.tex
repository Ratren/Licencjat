...
\begin{df}
Mówimy, że ciąg wektorów $v_1, v_2, \ldots, v_k$ z przestrzeni $V$ jest liniowo niezależny, gdy równanie $$a_1v_1 + a_2v_2 + \ldots + a_kv_k = 0,$$ gdzie $\mathbf{0}$ oznacza wektor zerowy, jest spełnione tylko w przypadku gdy $a_i = 0$ dla $i = 1, 2, \ldots, k.$
\end{df}
...
Pokażemy, że $P$ tworzy bazę $\mathbb{R}^n.$ Załóżmy, że istnieje kombinacja liniowa układu $P$ dająca wektor zerowy:
$$\alpha_1p_1 + \alpha_2p_2 + \ldots + \alpha_np_n = \mathbb{0}.$$
Aby pokazać, że $\alpha_i = 0$ dla wszystkich $i=1,\ldots,n$ przemnóżmy powyższą kombinację obustronnie przez $p_i^TA.$
$$p_i^TA(\alpha_1p_1 + \alpha_2p_2 + \ldots + \alpha_np_n) = p_i^TA\mathbb{0} = \mathbb{0}$$
Wykorzystując wzajemną sprzężoność wektorów $p_i^TAp_j = 0$ dla $i \neq j$ otrzymujemy:
$$\alpha_ip_i^TAp_i.$$ 
Ponieważ $p_i^TAp_i \neq 0,$ można zapisać $\alpha_i = 0.$ 
